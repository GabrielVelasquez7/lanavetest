...
interface BanqueoPrizesDolaresProps {
  form: UseFormReturn<BanqueoForm>;
  lotteryOptions: LotterySystem[];
}

export const BanqueoPrizesDolares = ({ form, lotteryOptions }: BanqueoPrizesDolaresProps) => {
  ...
  const handleInputBlur = (systemId: string, index: number) => {
    ...
    form.setValue(`systems.${index}.prizes_usd`, numValue, { shouldDirty: true, shouldValidate: false });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Premios en Dólares</CardTitle>
      </CardHeader>
