...
interface BanqueoSalesBolivaresProps {
  form: UseFormReturn<BanqueoForm>;
  lotteryOptions: LotterySystem[];
}

export const BanqueoSalesBolivares = ({ form, lotteryOptions }: BanqueoSalesBolivaresProps) => {
  ...
  const handleInputBlur = (systemId: string, index: number) => {
    ...
    form.setValue(`systems.${index}.sales_bs`, numValue, { shouldDirty: true, shouldValidate: false });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Ventas en Bolívares</CardTitle>
      </CardHeader>
